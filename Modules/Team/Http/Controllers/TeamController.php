...
class TeamController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $teams = Team::active()->paginate($per_page);
    ...
    public function create(Request $request)
    {
        ...
        if(isset($request->department_id) && $request->department_id != 0){
            /*for select department*/
            $leaders = User::active()->where('department_id', $request->department_id)->pluck('email', 'id');
            $leaders->prepend('--Select options--');

            $choosen_department_id = $request->department_id;

        }else{
            /*for select department*/
            $leaders = User::active()->pluck('email', 'id');
            $leaders->prepend('--Select options--');
    ...
    public function edit($id)
    {
      ...
      /*for select department*/
        $leaders = User::active()->where('department_id', $team->department->id)->pluck('email', 'id');
        $leaders->prepend('--Select options--');
